...
Obj.prototype.setLogger = function () {
    var self = this;

    self.logger = {};

    ['error', 'info', 'debug'].forEach(function (level) {
        if (typeof self.opts.logger[level] === 'function') {
            self.logger[level] = self.opts.logger[level];
        } else {
